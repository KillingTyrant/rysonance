---
name: supabase-schema-workflow
description: Workflow completo per modificare lo schema del database di questo progetto - generare migrazioni dai file dichiarativi in supabase/schemas/, ciclo di sviluppo contro lo stack Supabase locale in Docker, ordine di esecuzione dei file, entità che il diff dichiarativo non gestisce e vanno in una migrazione scritta a mano, contenuti del catalogo in supabase/seeds/, checklist di sicurezza RLS. Da caricare per qualsiasi lavoro su tabelle, colonne, funzioni, policy RLS, migrazioni, seed o dati di catalogo.
---

# Workflow schema Supabase
...
Le tre proibizioni fondamentali (non scrivere migrazioni a mano, non modificare lo schema
direttamente sul DB, le migrazioni committate sono immutabili) stanno in `CLAUDE.md` nella
root: valgono sempre, anche senza questa skill caricata.
...
2. **Genera e rivedi la migrazione** prima di committare:
   ```bash
...
3. **Committa insieme** il file in `schemas/` e la migrazione generata. Una PR con una
   migrazione senza il corrispondente cambio in `schemas/` non è valida.
4. Per correggere una migrazione già committata si genera una **nuova** migrazione. In
   sviluppo locale si può ripartire con `npx supabase db reset` /
...
- **Studio locale (`:54323`) è per ispezionare, non per modificare lo schema.** Ogni modifica
  fatta lì va persa al primo `db reset` e non finisce in `schemas/`.
